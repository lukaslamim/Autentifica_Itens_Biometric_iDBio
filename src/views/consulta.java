...
public final class consulta extends javax.swing.JFrame {
    ...
    public consulta() {
        ...
        if (ConnectionBD.FazerConeccao() == null) {
            ...
            //usar somente se o banco resetar
            //banco.UploadSom();
            //baixa som do banco
            ...
            while (true) {
                ...
                switch (retName) {
                    case "ERROR_NO_FINGER_DETECTED" -> {
                        alerta_centro.setText("");
                        alerta_cima.setText("CONECTADO");
                        ...
                        if (digitalExiste(digital_capturada)) {
                            ...
                            if (liberacao(digital_capturada)) {
                                ...
                                int id_colaborador = banco.getColaborador(banco.getDados(getInfo(digital_capturada)[1]).getId()).getId();
                                fazerBaruilhoNotLiberado(banco.getTabelaDados(id_colaborador).getNome());
                            }
                        } else {
                            alerta_baixo.setText("");
                            alerta_cima.setText("");
                            alerta_centro.setText("SEM CADASTRO");
